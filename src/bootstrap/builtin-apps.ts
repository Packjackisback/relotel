...
const rawModules = import.meta.glob('../apps/*.tsx', { eager: true, as: 'raw' }) as Record<string, string>;
// Runtime modules to access appInfo
const modModules = import.meta.glob('../apps/*.tsx', { eager: true }) as Record<string, any>;
...
export async function bootstrapBuiltInApps() {
  ...
  for (const [path, tsxCode] of Object.entries(rawModules)) {
    ...
    const id = info.id;
    const dir = `/apps/${id}`;
    // (Re)write the files every time so that changes during development propagate.
    if (!existing.has(id)) {
    ...
    const { code: transpiled } = Babel.transform(tsxCode, {
      presets: [
        'typescript',
        // Use classic runtime to avoid relying on jsx-runtime import which would need bundling
        ['react', { runtime: 'classic' }],
      ],
      // Transform the (potential) ES module into UMD so we can stub require/exports easily
      plugins: [
